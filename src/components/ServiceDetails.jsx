...
const validateServiceDetails = (user) => {
  if (
    user.noOfCleaners === undefined ||
    user.cleaningHours === undefined ||
    user.cleaningFrequency === undefined ||
    user.needMaterials === undefined
  ) {
    // console.log(user);
    return false;
  }
...
export default function ServiceDetails() {
  ...
  const [activeMaterialButton, setActiveMaterialButton] = useState(
    user.needMaterials != undefined && (user?.needMaterials ? "Yes" : "No")
  );
  ...
  return (
    <>
      <img className="m-0 w-full rounded-top-lg" src={CleaningImg} />
